...
void DiffusionRelaxation<HeatInner<KernelGradientType>, DiffusionType>::interaction(size_t index_i, Real dt)
{
    for (size_t m = 0; m < this->diffusions_.size(); ++m)
    {
        ...
        (*this->heat_flux_inner_dt_[m])[index_i] += d_species / rho_i / c_v_i;
    }
}
//=================================================================================================//
template <class KernelGradientType, class DiffusionType>
...
void DiffusionRelaxation<HeatInner<KernelGradientType>, DiffusionType>::update(size_t index_i, Real dt)
{
    ...
    for(size_t m = 0; m < this->diffusions_.size(); ++m)
	{
		...
		Real rho_i = this->diffusions_[m]->getDensity();
		Real c_v_i = this->diffusions_[m]->getSpecificHeat();
        (*this->heat_flux_inner_[m])[index_i] = dt * c_v_i * (*this->heat_flux_inner_dt_[m])[index_i];
	}
}
//=================================================================================================//
//template <class DiffusionType>
...
void DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::interaction(size_t index_i, Real dt)
{
     for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
     {
         ...
         for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
         {
             ...
             const Vecd &grad_ijV_j = this->contact_kernel_gradients_[k](index_i, index_j, dW_ijV_j, e_ij);
             Real area_ij = 2.0 * grad_ijV_j.dot(e_ij) / r_ij_;
             getDiffusionChangeRateTwoPhaseContact(index_i, index_j, e_ij, area_ij, gradient_species_k, heat_flux_contact_dt_k);
         }
     }
 }
//=================================================================================================//
template <class ContactKernelGradientType, class DiffusionType, class ContactDiffusionType>
        ...
        const StdVec<StdLargeVec<Real> *> &gradient_species_k, StdVec<StdLargeVec<Real> *> &heat_flux_contact_dt_k)
 {
    for (size_t m = 0; m < this->diffusions_.size(); ++m)
    {
        ...
        (*heat_flux_contact_dt_k[m])[particle_i] += diff_coeff_ij * phi_ij * surface_area_ij / rho_i / c_v_i;
    }
 }
 //=================================================================================================//
 template <class ContactKernelGradientType, class DiffusionType, class ContactDiffusionType>
 ...
 void DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::update(size_t index_i, Real dt)
 {
     for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
     {
         for (size_t m = 0; m < this->diffusions_.size(); ++m)
         {
             ...
             (*heat_flux_contact_[k][m])[index_i] = dt * specific_heat_i * (*heat_flux_contact_dt_[k][m])[index_i];
         }
     }
 }
 //=================================================================================================//
