      ...
      Vol_(*this->particles_->template getVariableDataByName<Real>("VolumetricMeasure"))
{
    for (auto &diffusion : diffusions_)
    {
        ...
        /*for debuging*/
        this->particles_->template addVariableToSort<Real>(diffusion_species_name + "ChangeRate");
        this->particles_->template addVariableToWrite<Real>(diffusion_species_name + "ChangeRate");
      ...
      kernel_gradient_(this->particles_)
{
    for(size_t m = 0; m < this->diffusions_.size(); ++m)
	{
        auto diffusion = this->diffusions_[m];
		std::string diffusion_species_name = diffusion->DiffusionSpeciesName();
        ...
        this->particles_->template addVariableToSort<Real>(diffusion_species_name + "FluxInnerChangeRate");
...
void DiffusionRelaxation<HeatInner<KernelGradientType>, DiffusionType>::initialization(size_t index_i, Real dt)
{
    ...
    for (size_t m = 0; m < this->diffusions_.size(); ++m)
    {
        (*this->heat_flux_inner_dt_[m])[index_i] = 0.0;
    }
}
//=================================================================================================//
template <class KernelGradientType, class DiffusionType>
void DiffusionRelaxation<HeatInner<KernelGradientType>, DiffusionType>::update(size_t index_i, Real dt)
{
    ...
    for(size_t m = 0; m < this->diffusions_.size(); ++m)
	{
		...
		//Real rho_i = this->diffusions_[m]->getDensity();
		//Real c_v_i = this->diffusions_[m]->getSpecificHeat();
        (*this->heat_flux_inner_[m])[index_i] = (*this->heat_flux_inner_dt_[m])[index_i];
	}
}
//=================================================================================================//
//template <class DiffusionType>
//HeatGetDiffusionTimeStepSize<DiffusionType>::
//    HeatGetDiffusionTimeStepSize(SPHBody &sph_body, DiffusionType &diffusion)
//    : BaseDynamics<Real>(sph_body)
//{
//    Real smoothing_length = sph_body.sph_adaptation_->ReferenceSmoothingLength();
//    diff_time_step_ = diffusion.getDiffusionTimeStepSize(smoothing_length);
//}
//=================================================================================================//
template <class ContactDataDelegationType, class DiffusionType, class ContactDiffusionType>
template <typename BodyRelationType>
DiffusionRelaxation<ContactDataDelegationType, DiffusionType, ContactDiffusionType>::
    DiffusionRelaxation(BodyRelationType &contact_body_relation, StdVec<DiffusionType *> diffusions, StdVec<ContactDiffusionType *> contact_diffusions)
    : LocalDynamics(contact_body_relation.getSPHBody()), ContactDataDelegationType(contact_body_relation),
      diffusions_(diffusions), contact_diffusions_(contact_diffusions),
      Vol_(*this->particles_->template getVariableDataByName<Real>("VolumetricMeasure")) 
{
    for (auto &diffusion : diffusions_)
    {
...
template <typename BodyRelationType>
DiffusionRelaxation<ContactDataDelegationType, DiffusionType, ContactDiffusionType>::
    DiffusionRelaxation(BodyRelationType &contact_body_relation, DiffusionType *diffusion, ContactDiffusionType *contact_diffusion)
    : DiffusionRelaxation<DataDelegateContact, DiffusionType, ContactDiffusionType>(contact_body_relation, StdVec<DiffusionType *>{diffusion}, StdVec<ContactDiffusionType *>{contact_diffusion}) {}
//=================================================================================================//
template <class ContactDataDelegationType, class DiffusionType, class ContactDiffusionType>
void DiffusionRelaxation<ContactDataDelegationType, DiffusionType, ContactDiffusionType>::initialization(size_t index_i, Real dt)
{
    /*for (size_t m = 0; m < diffusions_.size(); ++m)
    {
        (*diffusion_dt_[m])[index_i] = 0;
...
//=================================================================================================//
template <class ContactDataDelegationType, class DiffusionType, class ContactDiffusionType>
void DiffusionRelaxation<ContactDataDelegationType, DiffusionType, ContactDiffusionType>::update(size_t index_i, Real dt)
{
    /*for (size_t m = 0; m < diffusions_.size(); ++m)
    {
        (*diffusion_species_[m])[index_i] += dt * (*diffusion_dt_[m])[index_i];
...
DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::
    DiffusionRelaxation(Args &&...args)
    : DiffusionRelaxation<DataDelegateContact, DiffusionType, ContactDiffusionType>(std::forward<Args>(args)...)
{
    contact_gradient_species_.resize(this->contact_particles_.size());
    heat_flux_contact_.resize(this->contact_particles_.size());
    heat_flux_contact_dt_.resize(this->contact_particles_.size());
    //heat_flux_wu_contact_dt_.resize(this->contact_particles_.size());
    //heat_flux_wu_contact_.resize(this->contact_particles_.size());
    for (size_t k = 0; k != this->contact_particles_.size(); ++k)
    {
        BaseParticles *contact_particles_k = this->contact_particles_[k];
        contact_kernel_gradients_.push_back(ContactKernelGradientType(this->particles_, contact_particles_k));
        ...
        for (auto &diffusion : this->diffusions_)
        {
           ...
           /* heat_flux_wu_contact_dt_[k].push_back(
                this->particles_->template registerSharedVariable<Real>(diffusion_species_name + "FluxWuContactChangeRate"));
            this->particles_->template addVariableToSort<Real>(diffusion_species_name + "FluxWuContactChangeRate");
            this->particles_->template addVariableToWrite<Real>(diffusion_species_name + "FluxWuContactChangeRate");

            heat_flux_wu_contact_[k].push_back(
...
//=================================================================================================//
template <class ContactKernelGradientType, class DiffusionType, class ContactDiffusionType>
void DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::interaction(size_t index_i, Real dt)
{
     for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
     {
         StdVec<StdLargeVec<Real> *> &gradient_species_k = this->contact_gradient_species_[k];
         ...
         Neighborhood &contact_neighborhood = (*this->contact_configuration_[k])[index_i];
         StdVec<StdLargeVec<Real> *> &heat_flux_contact_dt_k = heat_flux_contact_dt_[k];
         //StdVec<StdLargeVec<Real> *> &heat_flux_wu_contact_dt_k = heat_flux_wu_contact_dt_[k];
         for (size_t n = 0; n != contact_neighborhood.current_size_; ++n)
         {
             size_t index_j = contact_neighborhood.j_[n];
             Real r_ij_ = contact_neighborhood.r_ij_[n];
             ...
             getDiffusionChangeRateTwoPhaseContact(index_i, index_j, e_ij, fector_ij, area_ij, cross_section, gradient_species_k, heat_flux_contact_dt_k);
         }
     }
 }
//=================================================================================================//
template <class ContactKernelGradientType, class DiffusionType, class ContactDiffusionType>
 void DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::
     getDiffusionChangeRateTwoPhaseContact(size_t particle_i, size_t particle_j, Vecd &e_ij, Real fector_ij, Real surface_area_ij, Real cross_section,
 ...
 void DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::
     getDiffusionChangeRateTwoPhaseContact(size_t particle_i, size_t particle_j, Vecd &e_ij, Real surface_area_ij, Real cross_section,
                                           const StdVec<StdLargeVec<Real> *> &gradient_species_k, StdVec<StdLargeVec<Real> *> &heat_flux_contact_dt_k, StdVec<StdLargeVec<Real> *> &heat_flux_wu_contact_dt_k)
 {
     for (size_t m = 0; m < this->diffusions_.size(); ++m)
     {
         Real A_i = (this->particles_->ParticleSpacing(particle_i)) * (this->particles_->ParticleSpacing(particle_i));
         ...
         //(*heat_flux_contact_dt_k[m])[particle_i] += diff_coeff_ij * phi_ij * fector_ij * cross_section;
         //(*heat_flux_wu_contact_dt_k[m])[particle_i] += diff_coeff_ij * phi_ij * fector_ij * cross_section;
     }
 }
 //=================================================================================================//
 template <class ContactKernelGradientType, class DiffusionType, class ContactDiffusionType>
 void DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::initialization(size_t index_i, Real dt)
 {
     for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
     {
         for (size_t m = 0; m < this->diffusions_.size(); ++m)
         {
             ...
             //(*heat_flux_wu_contact_dt_[k][m])[index_i] = 0.0;
         }
     }
 }
 //=================================================================================================//
 template <class ContactKernelGradientType, class DiffusionType, class ContactDiffusionType>
 void DiffusionRelaxation<Contact<ContactKernelGradientType>, DiffusionType, ContactDiffusionType>::update(size_t index_i, Real dt)
 {
     for (size_t k = 0; k < this->contact_configuration_.size(); ++k)
     {
         for (size_t m = 0; m < this->diffusions_.size(); ++m)
         {
             ...
             //(*heat_flux_wu_contact_[k][m])[index_i] = (*heat_flux_wu_contact_dt_[k][m])[index_i];
         }
     }
 }
 //=================================================================================================//
