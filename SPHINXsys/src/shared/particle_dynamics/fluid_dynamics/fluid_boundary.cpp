...
namespace SPH
{
	...
	namespace fluid_dynamics
	{
		...
		/*StaticConfinementExtendIntegration1stHalf::
			StaticConfinementExtendIntegration1stHalf(NearShapeSurface& near_surface, Real penalty_strength, Real sound_speed)
			: LocalDynamics(near_surface.getSPHBody()), FluidDataSimple(sph_body_),
			fluid_(particles_->fluid_), sound_speed_(sound_speed),
			rho_(particles_->rho_), p_(particles_->p_),
			pos_(particles_->pos_), vel_(particles_->vel_),
		...
		//	Real penalty_force = rho_[index_i] * sound_speed_ * sound_speed_;
		...
		//	Real penalty = ratio < 1.0 ? (1.0 - ratio) * (1.0 - ratio) * 0.5 * penalty_force : 0.0;
