...
int main(int ac, char *av[])
{
	...
	ball.defineAdaptation<ParticleRefinementNearSurface>(1.15, 1.0, 3);
	...
	(!sph_system.RunParticleRelaxation() && sph_system.ReloadParticles())
		...
		:ball.generateParticles<ParticleGeneratorMultiResolution>();
	ball.addBodyStateForRecording<Real>("SmoothingLengthRatio");
	...
	if (sph_system.RunParticleRelaxation())
	{
		...
		//----------------------------------------------------------------------
		AdaptiveInnerRelation ball_inner(ball);
		//----------------------------------------------------------------------
		//	Define the methods for particle relaxation for ball.
		//----------------------------------------------------------------------
		SimpleDynamics<RandomizeParticlePosition> ball_random_particles(ball);
		relax_dynamics::RelaxationStepInner ball_relaxation_step_inner(ball_inner, true);
		SimpleDynamics<relax_dynamics::UpdateSmoothingLengthRatioByShape> update_smoothing_length_ratio(ball);

		//----------------------------------------------------------------------
		//	Output for particle relaxation.
		//----------------------------------------------------------------------
		BodyStatesRecordingToVtp write_relaxed_particles(io_environment, sph_system.real_bodies_);
		MeshRecordingToPlt cell_linked_list_recording(io_environment, ball.getCellLinkedList());
		ReloadParticleIO write_particle_reload_files(io_environment, ball);
		//----------------------------------------------------------------------
		//	Particle relaxation starts here.
		//----------------------------------------------------------------------
		ball_random_particles.parallel_exec(0.25);
		ball_relaxation_step_inner.SurfaceBounding().parallel_exec();
	...
	AdaptiveInnerRelation ball_inner(ball);
	AdaptiveContactRelation ball_contact(ball, { &plate });
	AdaptiveContactRelation ball_observer_contact(ball_observer, { &ball });
	////----------------------------------------------------------------------
