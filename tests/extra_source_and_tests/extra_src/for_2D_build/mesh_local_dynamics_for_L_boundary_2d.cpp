...
void InitializeDataForSingularPackageLBoundary::update(const size_t package_index, Real far_field_level_set)
{
    ...
    mesh_data_.for_each_cell_data(
        [&](int i, int j)
        {
            ...
            kernel_gradient_multiply_Rij[i][j] = 0.0;
            kernel_gradient_divide_Rij[i][j] = 0.0;
        });
}
//=============================================================================================//
...
void WriteMeshFieldToPltLBoundary::update(std::ofstream &output_file)
{
    ...
    for (int j = 0; j != number_of_operation[1]; ++j)
    {
        for (int i = 0; i != number_of_operation[0]; ++i)
        {
            output_file << mesh_data_.DataValueFromGlobalIndex(kernel_gradient_multiply_Rij_, Arrayi(i, j))
                        << " ";
        }
        output_file << " \n";
    }
    ...
    for (int j = 0; j != number_of_operation[1]; ++j)
    {
        for (int i = 0; i != number_of_operation[0]; ++i)
        {
            output_file << mesh_data_.DataValueFromGlobalIndex(kernel_gradient_divide_Rij_, Arrayi(i, j))
                        << " ";
        }
        output_file << " \n";
    }
