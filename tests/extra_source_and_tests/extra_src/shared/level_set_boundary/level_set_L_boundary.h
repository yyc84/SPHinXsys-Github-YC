...
#include "all_mesh_dynamics_L_boundary.h"
 ...
 */
class MultilevelLevelSetLBoundary : public BaseMeshField
{
  public:
  MultilevelLevelSetLBoundary(BoundingBox tentative_bounds, Real reference_data_spacing, size_t total_levels, Shape &shape, SPHAdaptation &sph_adaptation);
  MultilevelLevelSetLBoundary(BoundingBox tentative_bounds, MeshWithGridDataPackagesType* coarse_data, Shape &shape, SPHAdaptation &sph_adaptation);
    ~MultilevelLevelSetLBoundary(){};

    void cleanInterfaceLBoundary(Real small_shift_factor);
    void correctTopologyLBoundary(Real small_shift_factor);
  ...
  protected:
    ...
    void registerProbesLBoundary(size_t level);
    ...
    StdVec<ProbeKernelGradientMultiplyRijIntegral *> probe_kernel_gradient_multiply_Rij_integral_set_;
    StdVec<ProbeKernelGradientDivideRijIntegral *> probe_kernel_gradient_divide_Rij_integral_set_;
    ...
    UniquePtrsKeeper<ProbeKernelGradientMultiplyRijIntegral> probe_kernel_multiply_Rij_integral_vector_keeper_;
    UniquePtrsKeeper<ProbeKernelGradientDivideRijIntegral> probe_kernel_divide_Rij_integral_vector_keeper_;

    UniquePtr<CleanInterfaceLBoundary> clean_interface_L_boundary;
