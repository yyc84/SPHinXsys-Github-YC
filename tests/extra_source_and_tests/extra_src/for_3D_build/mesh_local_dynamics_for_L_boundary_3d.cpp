...
{
//=============================================================================================//
size_t BaseMeshLocalDynamicsLBoundary::SortIndexFromCellIndex(const Arrayi &cell_index)
{
    return cell_index[0] * all_cells_[1] * all_cells_[2] + cell_index[1] * all_cells_[2] + cell_index[2];
}
//=============================================================================================//
Arrayi BaseMeshLocalDynamicsLBoundary::CellIndexFromSortIndex(const size_t &sort_index)
{
    Array3i cell_index;
...
//=============================================================================================//
void InitializeDataForSingularPackageLBoundary::update(const size_t package_index, Real far_field_level_set)
{
    ...
    auto &kernel_gradient_multiply_Rij = kernel_gradient_multiply_Rij_.Data()[package_index];
    auto &kernel_gradient_divide_Rij = kernel_gradient_divide_Rij_.Data()[package_index];
    ...
    mesh_data_.for_each_cell_data(
        [&](int i, int j, int k)
        {
            ...
            kernel_gradient_multiply_Rij[i][j][k] = 0.0;
            kernel_gradient_divide_Rij[i][j][k] = 0.0;
        });
}
//=============================================================================================//
...
// }
//=============================================================================================//
Real UpdateKernelIntegralsLBoundary::computeKernelIntegral(const Vecd &position)
{
...
//=============================================================================================//
Vecd UpdateKernelIntegralsLBoundary::computeKernelGradientIntegral(const Vecd &position)
{
    Real phi = probeSignedDistance(position);
    Real cutoff_radius = kernel_.CutOffRadius(global_h_ratio_);
...
//=============================================================================================//
Real UpdateKernelIntegralsLBoundary::computeKernelGradientMultiplyRijIntegral(const Vecd &position)
{
    Real phi = probeSignedDistance(position);
    Real cutoff_radius = kernel_.CutOffRadius(global_h_ratio_);
    Real threshold = cutoff_radius + data_spacing_;
...
// }
//=============================================================================================//
void WriteMeshFieldToPltLBoundary::update(std::ofstream &output_file)
{
    ...
    for (int k = 0; k != number_of_operation[2]; ++k)
        for (int j = 0; j != number_of_operation[1]; ++j)
        {
            for (int i = 0; i != number_of_operation[0]; ++i)
            {
                output_file << mesh_data_.DataValueFromGlobalIndex(kernel_weight_, Arrayi(i, j, k))
                            << " ";
            }
            output_file << " \n";
        }
    ...
    for (int k = 0; k != number_of_operation[2]; ++k)
        for (int j = 0; j != number_of_operation[1]; ++j)
        {
            for (int i = 0; i != number_of_operation[0]; ++i)
            {
                output_file << mesh_data_.DataValueFromGlobalIndex(kernel_gradient_, Arrayi(i, j, k))[0]
                            << " ";
            }
            output_file << " \n";
        }
    ...
    for (int k = 0; k != number_of_operation[2]; ++k)
        for (int j = 0; j != number_of_operation[1]; ++j)
        {
            for (int i = 0; i != number_of_operation[0]; ++i)
            {
                output_file << mesh_data_.DataValueFromGlobalIndex(kernel_gradient_, Arrayi(i, j, k))[1]
                            << " ";
            }
            output_file << " \n";
        }
    ...
    for (int k = 0; k != number_of_operation[2]; ++k)
        for (int j = 0; j != number_of_operation[1]; ++j)
        {
            for (int i = 0; i != number_of_operation[0]; ++i)
            {
                output_file << mesh_data_.DataValueFromGlobalIndex(kernel_gradient_, Arrayi(i, j, k))[2]
                            << " ";
            }
            output_file << " \n";
        }
    ...
    for (int k = 0; k != number_of_operation[2]; ++k)
        for (int j = 0; j != number_of_operation[1]; ++j)
        {
            for (int i = 0; i != number_of_operation[0]; ++i)
            {
                output_file << mesh_data_.DataValueFromGlobalIndex(kernel_gradient_multiply_Rij_, Arrayi(i, j, k))
                            << " ";
            }
            output_file << " \n";
        }
    ...
    for (int k = 0; k != number_of_operation[2]; ++k)
        for (int j = 0; j != number_of_operation[1]; ++j)
        {
            for (int i = 0; i != number_of_operation[0]; ++i)
            {
                output_file << mesh_data_.DataValueFromGlobalIndex(kernel_gradient_divide_Rij_, Arrayi(i, j, k))
                            << " ";
            }
            output_file << " \n";
        }
