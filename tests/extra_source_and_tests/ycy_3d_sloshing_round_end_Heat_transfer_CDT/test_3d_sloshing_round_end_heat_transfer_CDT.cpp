...
Real k_water_contact = 2 * k_water * k_air / (k_water + k_air);
Real k_air_contact = 2 * k_air * k_water / (k_air + k_water);
Real mu_f = 121.79e-6;
Real mu_a = 8.73e-6;
...
int main(int ac, char *av[])
{
   ...
   /* air_block.defineBodyLevelSetShape()->writeLevelSet(sph_system);
    ...
    HeatIsotropicDiffusion water_heat_diffusion_contact("Phi", "Phi", k_water_contact, rho0_f, c_p_water);
    HeatIsotropicDiffusion air_heat_diffusion_contact("Phi", "Phi", k_air_contact, rho0_a, c_p_air);

    ThermalRelaxationComplex thermal_relaxation_complex_water(
        ConstructorArgs(water_block_inner, &water_heat_diffusion),
        ConstructorArgs(water_air_contact, &water_heat_diffusion_contact));
    ThermalRelaxationComplex thermal_relaxation_complex_air(
        ConstructorArgs(air_block_inner, &air_heat_diffusion),
        ConstructorArgs(air_water_contact, &air_heat_diffusion_contact));
